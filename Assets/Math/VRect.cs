...
public struct VRect
{
	...
	public bool Contains(VInt3 point, bool allowInverse)
	{
		...
		bool flag = false;
		if (((float)this.width < 0f && point._x <= this.xMin && point._x > this.xMax) || ((float)this.width >= 0f && point._x >= this.xMin && point._x < this.xMax))
		{
			flag = true;
		}
		return flag && (((float)this.height < 0f && point._y <= this.yMin && point._y > this.yMax) || ((float)this.height >= 0f && point._y >= this.yMin && point._y < this.yMax));
	}
